...
class CaptioningRNN(object):
  ...
  def loss(self, features, captions):
    ...
    gradients['W_proj'] = np.dot(features.T, grad_h0)
    gradients['b_proj'] = np.sum(grad_h0, axis=0)
  ...
  def sample(self, features, max_length=30):
    ...
    for step in np.arange(max_length):
      ...
      for ind, caption in enumerate(current_captions):
        if np.array_equal(caption, self._end):
          mask[ind] = 0
      
      if np.sum(mask) == 0:
        break
      
      captions[:, step] = current_captions * mask
      word_embedding, _ = word_embedding_forward(current_captions, W_embed)
